...
const mockingoose = require('mockingoose').default; // acceder como objeto a dependencia
const { User } = require('../../../src/models');
let { UserModelMock: { users, user } } = require('../../mocks');

describe('User Repository Tests', () => {
    beforeEach(() => {
        mockingoose.resetAll(); // resetear todos los mocking en ese momento
        jest.clearAllMocks();
    });

    it('Should return a user by id', async() => {
        const _user = {...user }; // fotocopia de user(mock) sin referencia del archivo require('../../mocks');
        delete _user.password;
        mockingoose(User).toReturn(user, 'findOne'); // crea objeto(User) mongoose a partir de un mock (user)
